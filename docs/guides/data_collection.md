...
> Chinese: [data_collection_cn.md](data_collection_cn.md)
> Part of the [VLA training](../../README.md) project. Outputs land in `datasets/` and can be used by the training scripts.
...
The Pika SDK is **vendored in-tree** under [`../../collect/pika_sdk/`](../../collect/pika_sdk/) — no
...
- `pysurvive` — needed *only* if you drive the arm from the Vive tracker
  (`tracker_device: T20` in [`../../collect/configs/pika_config.yaml`](../../collect/configs/pika_config.yaml)).
...
1. **IP address** — set `robot.host` in the config to the controller's IP.
   The checked-in configs use placeholders; on the teach pendant go to
...
python preview_cameras.py --serial <camera-serial> # preview a specific camera
...
The vendored SDK ships under [`../../collect/pika_sdk/`](../../collect/pika_sdk/) (a verbatim copy of
