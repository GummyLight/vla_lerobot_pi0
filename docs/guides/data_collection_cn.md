...
> English: [data_collection.md](data_collection.md)
> 隶属于 [VLA training](../../README_CN.md) 项目。采集结果落到 `datasets/`，可直接用于训练脚本。
...
松灵提供的 Pika SDK 已按原样放在 [`../../collect/pika_sdk/`](../../collect/pika_sdk/)。
本项目只调用它，不重新授权 SDK 内容；使用和再分发请遵循原来源的许可要求。
...
1. **IP 地址** — 在 yaml 里把 `robot.host` 改成控制器实际 IP。仓库里的
   配置使用占位值；在示教器 *Settings → Network* 里看真实 IP。
...
python preview_cameras.py --serial <camera-serial> # 预览指定相机
