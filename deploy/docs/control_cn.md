...
> English: [deploy/docs/control.md](control.md)

本文说明 [deploy/run_pi0_robot.py](../run_pi0_robot.py) 在真机上是怎么从「策略输出」一路走到「机械臂动 + 夹爪开合」的。理解这个之后,调参 / 排错才有方向。
...
我们的实现 [deploy/run_pi0_robot.py:467](../run_pi0_robot.py#L467) 把每一轮的实际墙钟周期 `prev_loop_dt` 喂给下一轮的 `servoJ time`:
...
我们的封装 [deploy/run_pi0_robot.py:238-251](../run_pi0_robot.py#L238-L251):
...
第一次部署常见的坑:夹爪上电了、URCap 也装了,但没在 PolyScope 上跑一个含 Robotiq toolbar 的 program → `connection refused`。 [deploy/preflight_check.py](../preflight_check.py) 的第 3 项检查就是抓这个的。
...
## 调参速查表(改 [deploy/configs/run_pi0_robot.yaml](../configs/run_pi0_robot.yaml))
...
| 任务完全不会做 | **欠训** | `train/train_pi0.py --steps=30000 --resume=true` |
